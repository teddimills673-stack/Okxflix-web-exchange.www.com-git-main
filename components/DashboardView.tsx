      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-slate-900/90 border border-slate-800 rounded-2xl p-6 shadow-xl flex flex-col">
          ...
          <div className="h-[280px] w-full mt-2 flex items-center justify-center relative">
            {mounted && portfolioHistory.length > 0 ? (
              <div className="w-full h-full flex flex-col justify-between">
                <svg className="w-full h-[220px] overflow-visible" viewBox="0 0 600 180" preserveAspectRatio="none">
                  <defs>
                  ...
                  {(() => {
                    ...
                    const points = portfolioHistory.map((p, i) => {
                      const x = (i / (portfolioHistory.length - 1)) * 600;
                      const y = 160 - ((p.value - min) / range) * 140;
                      return { x, y, ...p };
                    });
                      ...
                      <>
                        ...
                        {points.map((pt, idx) => (
                          <g key={idx} className="group cursor-pointer">
                            <circle cx={pt.x} cy={pt.y} r="3.5" className="fill-blue-500 hover:scale-150 transition-transform" />
                            <title>{`${pt.date}: $${pt.value.toLocaleString()}`}</title>
                          </g>
                        ))}
                      </>
                    );
                ...
                <div className="flex justify-between items-center text-[11px] text-slate-400 font-mono px-1 border-t border-slate-800/60 pt-2">
                  <span>{portfolioHistory[0]?.date}</span>
                  <span className="text-blue-400 font-bold">Interactive Portfolio Valuation</span>
